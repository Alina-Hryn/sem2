...
public class Kernel {
    ...
    public static int creat(String pathname, short mode)
        ...
        if (indexNodeNumber < 0) {
            ...
            System.out.println(process.getUid() + "      " + process.getGid());
...
/*
    private static int fileCurrentUserMode(IndexNode indexNode) {
        int i = (int) indexNode.getMode();
        int user = (i & Kernel.S_IRWXU) >> 6;
        int group = (i & Kernel.S_IRWXG) >> 3;
        int others = i & Kernel.S_IRWXO;
        if (process.getUid() == user)
            return 1;
        if (process.getGid() == group)
            return 2;
        else
            return 3;
    }

    private static boolean fileReadable(IndexNode indexNode) {

    }

    private static boolean fileWritable(IndexNode indexNode) {
...
*/
    ...
    public static int chmod(String name, short mode)
        ...
        if (fd < 0) {
            System.err.println(sys_errlist[2]);
            return -1;
        }
    ...
    public static void initialize() {
        ...
        if (propertyFileName == null)
            propertyFileName = "C:\\moss\\filesys\\filesys.conf";
        ...
        String rootFileSystemFilename =
                properties.getProperty("filesystem.root.filename", "C:\\moss\\filesys\\filesys.dat");
